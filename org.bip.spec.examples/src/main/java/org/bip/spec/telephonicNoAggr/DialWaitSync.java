...
@ComponentType(initial = "s0", name = "org.bip.spec.telephonicNoAggr.DialWaitSync")
public class DialWaitSync implements ClientCaller {
	...
	public void dial(@Data(name = "dialerId") Integer x,
			@Data(name = "waiterId") Integer y) {
		System.err.println("a"+callNumber + " "+ dialerIds + waitersIds + x+talkShouldBeDeleted);
		// if x has been discarded because it is already been called, return

		if (talkShouldBeDeleted.compareAndSet(x - 1, 1, 0)) {
			return;
		}
		// if y has notified, remove y waiting, x waiting and y dialing
		if (waitersIds.get(y - 1) != 0) {
			// if x hasn't notified of waiting yet, we need to discard its future notification
			...
			clientActors.get(x).inform("dial", dataMap);
			clientActors.get(y).inform("wait", dataMap);
		...
		System.err.println("b"+callNumber + " "+ dialerIds + waitersIds+x+talkShouldBeDeleted);
	}
	...
	public void waitCall(@Data(name = "waiterId") Integer y) {
		System.err.println("c"+callNumber + " "+ dialerIds + waitersIds+y+talkShouldBeDeleted);
		...
		if (dialer > 0) { // need to send informs
			...
			dataMap.put("callId", callNumber.getAndIncrement());
			clientActors.get(y).inform("wait", dataMap);
			clientActors.get(dialer).inform("dial", dataMap);
			// callerAgregationExecutor.inform("dialDown", dataMap);
			// calleeAgregationExecutor.inform("waitDown", dataMap);
		} else { // y hasn't notified, store that y wants to talk
			waitersIds.set(y - 1, 1);
		}
		System.err.println("d"+callNumber + " "+ dialerIds + waitersIds+y+talkShouldBeDeleted);
	}
